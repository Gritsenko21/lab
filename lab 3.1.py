class Book:
    ...
    @property
    def name(self) -> str:
        """Название книги (неизменяемое)."""
        return self._name

    @property
    def author(self) -> str:
        """Автор книги (неизменяемое)."""
        return self._author
...
class AudioBook(Book):
    ...
    def duration(self) -> float:
        """Продолжительность аудиокниги в часах (положительное число с плавающей точкой)."""
        return self._duration

    @duration.setter
    def duration(self, value: float):
        if not isinstance(value, (int, float)):
            raise TypeError("Продолжительность должна быть положительным числом с плавающей точкой.")
        if value <= 0:
